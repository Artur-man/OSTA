...
Traditional power analysis estimates the sample size needed to detect a change in mean expression. In spatial omics, we must also consider the **spatial arrangement** of cells and features. **Spatial Power Analysis** [@Imkeller2023] provides a framework to determine the optimal sampling strategy:

1.  **Tissue Composition:** How many cells of a specific type are needed to detect a rare interaction?
2.  **Field of View (FOV) Selection:** What size and number of FOVs are required to capture spatial heterogeneity?
3.  **In Silico Tissue (IST):** Researchers can use IST models to simulate tissue structures and optimize their design before performing the actual experiment [@Imkeller2023].

## Panel and ROI Design

### Panel Design for Targeted Platforms

For imaging-based platforms (e.g., Xenium, CosMx, MERFISH), selecting the right gene panel is a critical experimental step [@Baran2023-marker; @Kuemmerle2024-probe-set].

*   **Exploratory vs. Hypothesis-driven:** Panels should include "anchor" markers for common cell types and "exploratory" targets.
*   **Density and Crosstalk:** Higher plexity can increase background noise and optical crosstalk [@Kuemmerle2024-probe-set].

### Region of Interest (ROI) Selection for Microdissection

Technologies like **NanoString GeoMx Digital Spatial Profiler (DSP)** rely on the user to select specific Regions of Interest (ROIs) for molecular profiling. Unlike whole-tissue imaging, these platforms use microdissection (e.g., via UV-cleavable barcodes) to collect data from targeted areas [@Beechem2020]. Selection strategies include:
...
- **Gridding:** Placing geometric shapes at regular intervals for unbiased sampling across a large area.
...
(usually 0.6 mm to 2.0 mm in diameter) on a single slide, offering a means to 
