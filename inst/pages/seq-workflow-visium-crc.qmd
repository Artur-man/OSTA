...
```{r ol}
# tabulate # and % of cells that'd be 
# discarded for different reasons
th <- metadata(spe)$qc$thresholds
ol <- data.frame(
    low_sum=spe$sum < th$sum,
    low_detected=spe$detected < th$detected,
    high_mt_prop=spe$mt_prop > th$subset.proportion,
    discard=spe$discard)
data.frame(
    check.names=FALSE,
    `#`=apply(ol, 2, sum), 
