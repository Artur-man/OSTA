...
Next, apply methods from `r BiocStyle::Biocpkg("scrapper")`. This stores the selected HVGs in `rowData(spe)$hvg`, which can be used as the input for subsequent steps. Here, we select the top 10% of genes. Alternatively, the `top` argument can be used to select a fixed number of top HVGs, say 1000 or 2000.
